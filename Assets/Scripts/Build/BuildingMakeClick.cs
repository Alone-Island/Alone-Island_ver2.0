...
public class BuildingMakeClick : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        //manualBuilding = FindObjectOfType<ManualBuilding>();
    }
    ...
    {
        isClicked = true;
        dragBuilding = PrefabUtility.InstantiatePrefab(Resources.Load("Prefabs/Building/FenceBuilding") as GameObject) as GameObject;      // C : 클릭한 건물 오브젝트 복사
        //dragBuilding.AddComponent<BuildingMakeClick>();
    ...
    private void OnMouseUp()
    {
        ...
        if (buildYes.activeSelf == true)
        {
            ...
            int index = this.gameObject.name == "Fence" ? 0 : 0;
            BuildingManager.FinalConsume(BuildingManager.buildingData[index]);
            // 2. 해당 위치에 '건물 설치'
            GameObject real = PrefabUtility.InstantiatePrefab(Resources.Load("Prefabs/Building/Fence") as GameObject) as GameObject;
            real.transform.localPosition = new Vector3(0, 0, 0);
            real.transform.position = dragBuilding.transform.position;
        ...
        else if (buildNo.activeSelf == true)
        {
            // 1. 건설 실패 모달창 띄우기
